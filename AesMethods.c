...
unsigned char* cipher(char* input, unsigned char* output) {
    ...
    int paddLength = padding(input);//padding done

	output = malloc(sizeof(unsigned char)*strlen(input));
	unsigned char hexString[strlen(input)*2 + 1];
	stringToHex(input, hexString);
	stringToBytes(hexString, hexString);//flag
	
	unsigned char key[] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};//2b7e151628aed2a6abf7158809cf4f3c
    ...
    return output;
//after encoding 'rakin' -> 77 eb df ce 9a f0 fe fe a9 dc 1c 56 8f d4 6b 9a
} 
...
void stringToHex(char* input,unsigned char* output) {//check unsigned char vs char str. if needed
    int loop;
    int i;
